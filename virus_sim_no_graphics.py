...
class Person:
    def __init__(self, x, y, v, phi, om, state, tp_spot, tp_radius, death_risk):
        self.x = x
        self.y = y
        self.vel = v
        self.angle = phi
...
class Population:
    def __init__(self, n, area, standard_distance, standard_velocity, infected_ratio, death_risk):
        self.size = 0
        self.distribution = {}
    ...
    def add_person(self, x, y, sick):
        p = Person(x, y, self.vel, 2 * np.pi * rnd.random(), self.rot_vel, sick, self.area.tp_spot, self.area.tp_radius, self.death_risk)
